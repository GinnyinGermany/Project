...
def calculate_event_synchronization_exact(t_i, t_j, tau_max):
    ...
    tau_i[0] = t_i[1] - t_i[0]
    tau_i[-1] = t_i[-1] - t_i[-2]
    np.minimum(t_i[1:-1] - t_i[:-2], t_i[2:] - t_i[1:-1], out=tau_i[1:-1])
    
    tau_j = np.empty(n_j)
    tau_j[0] = t_j[1] - t_j[0]
    tau_j[-1] = t_j[-1] - t_j[-2]
    np.minimum(t_j[1:-1] - t_j[:-2], t_j[2:] - t_j[1:-1], out=tau_j[1:-1])
...
def worker_function(s_pair, time_steps, n_surr, p_level, t_max):
    ...
    """
    s1, s2 = s_pair
    fake_scores_12 = np.empty(n_surr)
    fake_scores_21 = np.empty(n_surr)
